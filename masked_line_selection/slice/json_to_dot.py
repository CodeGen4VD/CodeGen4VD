...
def generate_sub_json(all_data_nodes, _point_slice_list, sub_graph_path, dict_path, func_name, points_name, label_dict):
    if sub_graph_path[-1] != '/':
        sub_graph_path += '/'
    ...
    if func_name.startswith("1_"):
        ...
        label_list = label_dict[func_name[2:]+'.c']
    ...
    for subgraph in _point_slice_list:
        ...
        json_paht = dict_file_path + func_name + "#" + str(iter) + '.json'
        if os.path.exists(dot_path):
            return
