...
def train(model, dataset, max_steps, dev_every, loss_function, optimizer, save_path, log_every=50, max_patience=5):
    ...
    try:
        for step_count in range(max_steps):
            ...
            if step_count % dev_every == (dev_every - 1):
                valid_loss, valid_f1 ,valid_acc, valid_pre, valid_rec= evaluate_loss(model, loss_function, dataset.initialize_valid_batch(),
                                                     dataset.get_next_valid_batch)
                step.append(step_count)
                t_loss.append(batch_loss.detach().cpu().item())
                v_loss.append(valid_loss)
                f11.append(valid_f1)
                pre.append(valid_pre)
                rec.append(valid_rec)
                accc.append(valid_acc)
                if valid_acc > 70.0 and valid_f1 > 70.0:
                    ckptname = str(dataset.batch_size) +'-' + str(valid_acc)+'-'+str(valid_pre)+'-'+str(valid_rec)+ '-' + str(valid_f1) +'-model.ckpt'
                    _save_ckpt_file = open('/home/pc/mVulPreter/slice_level_model/models/' + ckptname, 'wb')
    ...
    ckptname = '-' + str(best_acc) +'-'+str(best_pre)+'-'+str(best_rec)+ '-' + str(best_f1)+'-model.ckpt'
...
def predict_slice(model, loss_function, num_batches, data_iter):
    model.eval()
    with torch.no_grad():
        all_predictions, all_targets = [], []
        for i in range(num_batches):
            ...
            with open('./data_loader/slice_probability.txt', 'a') as wp:
                for index, slice_name in enumerate(slice_names):
                    wp.write(f'{slice_name}:{slice_scores[index]}\n')
    
        # print(accuracy_score(all_targets, all_predictions) * 100, \
        #         precision_score(all_targets, all_predictions) * 100, \
        #         recall_score(all_targets, all_predictions) * 100, \
        #         f1_score(all_targets, all_predictions) * 100)
