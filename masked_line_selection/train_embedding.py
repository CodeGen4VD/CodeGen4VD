...
def train_word_embedding(config_path: str):
    ...
    #with open(train_json, "r") as f:
        #paths = json.load(f)
    train_path ='/home/pc/dataset/big-vul/4_slices/vul/'
    func_list = os.listdir(train_path)
    paths = []
    for func_name in func_list:
        func_path = os.path.join(train_path,func_name)
    ...
    model = Word2Vec(sentences=tokens_list, min_count=3,seed=202407, vector_size=config.gnn.embed_size,
                     max_vocab_size=config.dataset.token.vocabulary_size, workers=num_workers, sg=1)
    model.wv.save("/home/pc/mVulPreter/preprocess/big_vul_w2v.wv")
